...
public class AlertHelper {
	...
	@Keyword
	def verifyAlertMessage(String expectedMessage) {
		if (WebUI.verifyAlertPresent(2, FailureHandling.OPTIONAL)) {
			String alertText = WebUI.getAlertText()

			WebUI.comment("⚠️ Alert muncul: " + alertText)

			// Verifikasi isi alert sesuai parameter expectedMessage
			WebUI.verifyMatch(alertText.toLowerCase(), expectedMessage.toLowerCase(), false)
